...
internal class AVLTree<T> : IEnumerable<T> where T : IComparable
{
    ...
    // <param name="enableNodeLookUp">Enabling lookup will hasten deletion/insertion/exists operations
    ...
    // Time complexity: O(log(n))
    // </summary>
    // CBS Integration note.
    // I added a return value to Insert.
    // It was throwing an error if you inserted a value that was already there.
    // I changed it to return the already existing value.
    // This allows a T that has a fuzzy (i.e. with EPS) value to always return the same value for
    // values within EPS. This assumes the Values have a CompareTo that takes the EPS into account.
    ...
    // Time complexity: O(log(n))
    // </summary>
    private T insert(AVLTreeNode<T> node, T value)
    {
        ...
        {
            return node.Value; // No need to updateHeight, etc.
        }
